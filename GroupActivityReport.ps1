...
$TextFileName = "C:\temp\" + $startTimeISO + "_log.text"
Start-Transcript -Path $TextFileName

$Report = [System.Collections.Generic.List[Object]]::new()
$ReportFile = "C:\temp\" + $startTimeISO + "_GroupsActivityReport.html"
$CSVFile = "C:\temp\" + $startTimeISO + "_GroupsActivityReport.csv"
...
Write-Host "Extracting list of Microsoft 365 Groups for checking at $GroupCheckStartTime"
...
#CLS #Clears text printed above before running audit
# Set up progress bar
$ProgDelta = 100/($GroupsCount); $CheckCount = 0; $GroupNumber = 0

#Indiviual case test code#


#####
#output all powershell output to txt file
...
ForEach ($Group in $Groups) { #Because we fetched the list of groups with Get-Recipient, the first thing is to get the group properties
   try {
         ...
         $GroupTime = Get-Date -Format "yyyyMMdd HHmmss" 
         Write-Host $GroupTime "Processing group" $G.DistinguishedName $G.objectID "`n"
...
Write-Host " "
Write-Host "Summary report in" $ReportFile "and CSV in" $CSVFile "PowerShell log in" $TextFileName

$endTime = Get-Date
Write-Host "`n End Time:   $($endTime.ToString("yyyy-MM-dd HH:mm:ss")) `n"
$runTime = New-TimeSpan -End $endTime -Start $startTime
Write-Host "Script ran for" $runTime.toString("mm' minutes 'ss' seconds'") "`n"
