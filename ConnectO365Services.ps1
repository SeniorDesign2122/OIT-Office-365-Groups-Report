  ...
  #Module and Connection settings for AzureAD V1 (MSOnline module)
   MSOnline
   {
    ...
    if($Module.count -eq 0)
    {
     ...
     {  #No space between "AzureAD.Please" area or connection error will occur
   ...
   SharePointPnP
   {
    ...
    {
     Import-Module SharepointpnpPowerShellOnline -DisableNameChecking
     Connect-PnPOnline -Url https://$SharePointHostName.sharepoint.com -credential $credential -WarningAction Ignore #https://wmutest1.sharepoint.com
    } 
    If ($? -eq $true)
    {
     if($ConnectedServices -ne "")
   ...
   Teams
   {
    $Module=Get-InstalledModule -Name MicrosoftTeams -MinimumVersion 2.0.0
    if($Module.count -eq 0)
    {
     Write-Host Required MicrosoftTeams module is not available  -ForegroundColor yellow 
     $Confirm= Read-Host Are you sure you want to install module? [Y] Yes [N] No
